...
class UWA_Admin {
	...
	public function __construct() {
		...
		$this->woo_ua_auction_item_condition = array(
		...
		$uwa_nonce_check_admin = wp_create_nonce( 'uwa_nonce_check_admin' );
	...
	public function uwa_manage_auction_page_admin_notice() {
		$uwa_nonce_check_admin = wp_create_nonce( 'uwa_nonce_check_admin' );
		if ( ! isset( $uwa_nonce_check_admin ) || ! wp_verify_nonce( $uwa_nonce_check_admin, 'uwa_nonce_check_admin' ) ) {
		...
		<div class="notice notice-warning is-dismissible">
			<p>  
				<?php
				/* Translators: %s is the URL of the Auctions products page. */
				printf(
					wp_kses(
						__( 'You can Manage All Auctions via Products List <a href="%s" target="_blank">Click Here.</a>', 'ultimate-woocommerce-auction' ),
						array(
							'a' => array(
								'href'   => array(),
								'target' => array(),
	...
	public function uwa_admin_main_menu() {
		...
		add_submenu_page( 'uwa_general_setting', __( 'Addons', 'ultimate-woocommerce-auction' ), __( 'Addons', 'ultimate-woocommerce-auction' ), 'manage_options', 'uwa_pro_addons', array( $this, 'uwa_pro_addons_page_handler' ) );
	...
	public function uwa_pro_addons_page_handler() {
		include_once WOO_UA_ADMIN . '/uwa_pro_addons.php';
	}
	...
	public function uwa_options_product_tab_content() {
		...
		if ( ( method_exists( $product, 'get_type' ) && $product->get_type() == 'auction' ) &&
			...
			$woo_ua_auction_status_type = 'expired';
		}
					...
					<div class='options_group'>
						...
						woocommerce_wp_hidden_input(
							array(
								...
								'value' => $woo_ua_auction_status_type,
							)
						);
						...
						<p class="form-field"> 									
								...
								<input type="checkbox" disabled id="uwa_auction_selling_type_auction_disabled" name="uwa_auction_selling_type_auction_disabled" checked=""> Auction 										  
									
								<span style="margin-right:25px"> </span>  
								
								<input type="checkbox" disabled id="uwa_auction_selling_type_buyitnow_disabled" name="uwa_auction_selling_type_buyitnow_disabled" checked="">  Buy it now								

								<span style="margin-right:20px"> </span>  
								<a href="<?php echo esc_url( $GLOBALS['uwa_update_link']['pro_link'] ); ?>" target="_blank" class="update-pro-link">Upgrade to PRO</a>
						...
						?>
						<p class=" form-field woo_ua_auction_type_field">
							<label for="woo_ua_auction_type">Auction type</label>
							<select style="" id="woo_ua_auction_type" disabled name="woo_ua_auction_type" class="select short">
								<option value="normal">Normal</option><option value="reverse">Reverse</option>		
							</select>
							<span style="margin-right:20px"> </span>  
							<a href="
							<?php
							echo esc_url( $GLOBALS['uwa_update_link']['pro_link'] );
							?>
							" target="_blank" class="update-pro-link">Upgrade to PRO</a>
						</p>
						<?php
						...
						?>
						<p class="form-field uwa_auction_variable_bid_increment_field ">
							<label for="uwa_auction_variable_bid_increment">Variable Bid Increment ($)</label><input type="checkbox" checked disabled class="checkbox" style="" name="uwa_auction_variable_bid_increment" id="uwa_auction_variable_bid_increment" value="yes"> 
							<span style="margin-right:20px"> </span>  
							<a href="
							<?php
							echo esc_url( $GLOBALS['uwa_update_link']['pro_link'] );
						...
						<p class="form-field woo_ua_next_bids_field ">
							<label for="woo_ua_next_bids">Number of next bids</label><input type="text" class="short" style="" disabled name="woo_ua_next_bids" id="woo_ua_next_bids" value="10" placeholder=""> 
							<span style="margin-right:20px"> </span>  
							<a href="
							<?php
							echo esc_url( $GLOBALS['uwa_update_link']['pro_link'] );
						...
						?>
						<p class="form-field woo_ua_auction_start_date_field ">
							<label for="woo_ua_auction_start_date">Start Date</label><input type="text" class="datetimepicker hasDatepicker" disabled style="" name="woo_ua_auction_start_date" id="woo_ua_auction_start_date" value="<?php echo esc_attr( wp_date( 'Y-m-d H:i:s', strtotime( '+1 day', time() ), get_uwa_wp_timezone() ) ); ?>" placeholder=""><img class="ui-datepicker-trigger" src="<?php echo esc_url( home_url( '/wp-content/plugins/woocommerce/assets/images/calendar.png' ) ); ?>" alt="Select a time" title="Select a time"> 
							<a href="<?php echo esc_url( $GLOBALS['uwa_update_link']['pro_link'] ); ?>" target="_blank" class="update-pro-link">Upgrade to PRO</a>
						</p>
						<?php
						...
						?>
							...
							<p class="form-field uwa_auto_renew_enable_field ">
								<label for="uwa_auto_renew_enable">Enable Automatic Relist</label><input type="checkbox" checked class="checkbox" disabled style="" name="uwa_auto_renew_enable" id="uwa_auto_renew_enable" value="yes"> 
								<span style="margin-right:20px"> </span>  
								<a href="<?php echo esc_url( $GLOBALS['uwa_update_link']['pro_link'] ); ?>" target="_blank" class="update-pro-link">Upgrade to PRO</a>
							</p>
						...
						<div class="uwa_admin_current_time">
								...
								printf(
									esc_html__( 'Current Blog Time is %s', 'ultimate-woocommerce-auction' ),
									'<strong>' . esc_html( get_uwa_now_date() ) . '</strong>'
								);
								/* Translators: %s is Current Timezone. */
								printf(
									esc_html__( 'Timezone: %s', 'ultimate-woocommerce-auction' ),
									'<strong>' . esc_html( wp_timezone_string() ) . '</strong>'
								);
								/* Translators: %s is URL. */
								printf(
									esc_html__( '%1$sChange.%2$s', 'ultimate-woocommerce-auction' ),
									'<a href="' . esc_url( admin_url( 'options-general.php?#timezone_string' ) ) . '" target="_blank">',
									'</a>'
								);
								?>
															</div>						
	...
	public function uwa_process_product_option_field( $post_id ) {
		...
		if ( isset( $_POST['woo_ua_product_type'] ) && isset( $_POST['product-type'] ) ) {
			if ( sanitize_text_field( $_POST['woo_ua_product_type'] ) == 'auction' && sanitize_text_field( $_POST['product-type'] ) != 'auction' ) {
				...
				/*$rows_affected = $wpdb->query($wpdb->prepare("DELETE FROM {$table_usermeta} WHERE meta_key = %s AND meta_value = %d", $metakey, $post_id));*/
				...
				$rows_affected = $wpdb->delete( $table_usermeta, $where );

				// Clear cache for usermeta
				if ( $rows_affected !== false ) {
					wp_cache_delete( 'usermeta:' . $metakey . ':' . $post_id, 'usermeta' );
				}
	...
	function uwa_render_auction_log() {
		...
		<?php if ( ( $product_data->is_woo_ua_closed() === true ) and ( $product_data->is_woo_ua_started() === true ) ) : ?>
				...
				<p><?php esc_html_e( 'Auction has expired', 'ultimate-woocommerce-auction' ); ?></p>
				...
				<?php } elseif ( $product_data->get_woo_ua_auction_current_bider() ) { ?>
					...
					<a href='post.php?&action=edit&post=<?php echo esc_attr( $product_data->get_woo_ua_order_id() ); ?>'>
						...
						if ( $order ) {
							...
							?>
						<p><?php esc_html_e( 'Order has been made, order status is', 'ultimate-woocommerce-auction' ); ?>: 
						<a href='post.php?&action=edit&post=<?php echo esc_attr( $product_data->get_woo_ua_order_id() ); ?>'>
							<?php echo esc_html( $order_status ); ?></a><span>
							<?php
						}
					}
					?>
			...
			<div class="woo_ua" id="woo_ua_auction_history" v-cloak>
				<div class="uwa-table-responsive">
						...
						if ( ! empty( $woo_ua_auction_history ) ) :
							...
							<tr>
								<th><?php esc_html_e( 'Bidder Name', 'ultimate-woocommerce-auction' ); ?></th>
								<th><?php esc_html_e( 'Bidding Time', 'ultimate-woocommerce-auction' ); ?></th>
								<th><?php esc_html_e( 'Bid', 'ultimate-woocommerce-auction' ); ?></th>								
								<th class="actions"><?php esc_html_e( 'Actions', 'ultimate-woocommerce-auction' ); ?></th>
							</tr>
							...
							<tr>
								...
								<?php echo esc_html( uwa_user_display_name( $history_value->userid ) ); ?></a></td>
								<td class="bid_date"><?php echo esc_html( mysql2date( $datetimeformat, $history_value->date ) ); ?></td>
	...
	function admin_uwa_filter_restrict_manage_posts() {
		...
		if ( $post_type == 'product' ) {
			...
			?>
					...
					<option value=""><?php esc_html_e( 'Auction filter By ', 'ultimate-woocommerce-auction' ); ?></option>
					<?php
					$current_filter = isset( $_GET['uwa_filter'] ) ? sanitize_text_field( $_GET['uwa_filter'] ) : '';
					foreach ( $filter_values as $label => $value ) {
							printf(
								'<option value="%s"%s>%s</option>',
								esc_attr( $value ), // Escape the value attribute
								selected( $value, $current_filter, false ), // Use the selected() function for the selected attribute
								esc_html( $label ) // Escape the label
							);
					}
					?>
					</select>
	...
	function wp_ajax_admin_cancel_bid() {
			...
			$postid_id = intval($postid_id); // Sanitize integer value
			$cache_key = md5( 'number-gen' ); // Generate a unique cache key based on the query
			$maxbid = wp_cache_get( $cache_key, 'woo_ua_auction_logs' );
			if ( false === $maxbid ) {
				wp_cache_set( $cache_key, $maxbid, 'woo_ua_auction_logs' );
			}
			...
			if ($postid) {
					...
					$log = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}woo_ua_auction_log WHERE id=%d", absint($log_id)));
			...
			if (!is_null($log)) {
				...
				if (($log->bid == $product_data->get_woo_ua_auction_current_bid()) && ($log->userid == $product_data->get_woo_ua_auction_current_bider())) {
					...
					$query = $wpdb->prepare(
						"SELECT * FROM {$wpdb->prefix}woo_ua_auction_log WHERE auction_id = %d ORDER BY `date` DESC, `bid` DESC LIMIT 1, 1",
						$postid_id
					);
	...
	function uwa_auctions_status_columns_status( $column, $postid ) {
		...
		if ( $column == 'admin_auction_status' ) {
			...
			if ( $product_data->get_type() == 'auction' ) {
				...
				if ( $closed === true ) {
					?>
									<span style="color:red;font-size:20px"><?php esc_html_e( 'Expired', 'ultimate-woocommerce-auction' ); ?></span>
				<?php } else { ?>				
					<span style="color:green;font-size:20px"><?php esc_html_e( 'Live', 'ultimate-woocommerce-auction' ); ?></span>
					<?php
				}
			}
		}
